...
public class ReverseMastermind {
  ...
  public ReverseMastermind(int length, int colors) {
    ...
    //worst cases = 5101, 1150, 1501, 0010
  }
  ...
  public int[] getFeedback(int[] score, int[] guess) {
    ...
    //Guess is correct condition
    if (score[0] == 4) {
      for (int hh = 0; hh < length; hh++) {
        feedback[hh] = 0;
      }
    }
    //Special Condition for 2 black pegs and 2 white pegs where guess and feedback are the same.
    ...
    //Everything else
      ...
      while (status == 1) {
       ...
       // feedback [0] = 1;
        ...
        if (score[0] == newScore[0] && score[1] == newScore[1] && !(Arrays.equals(feedback, guess))) {
          status = 0;
        }
      }
    }
